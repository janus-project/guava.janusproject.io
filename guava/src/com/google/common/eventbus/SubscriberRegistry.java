...
final class SubscriberRegistry {
	 ...
	 *
	 * <p>
	 * The {@link CopyOnWriteArraySet} values make it easy and relatively lightweight to get an immutable snapshot of all current subscribers to an event without any locking.
	 */
	 ...
	 */
	SubscriberRegistry(EventBus bus, Class<? extends Annotation> annotation) {
		this.bus = checkNotNull(bus);
		this.subscriberAnnotation = annotation;
	}

	SubscriberRegistry(Class<? extends Annotation> annotation) {
		this.subscriberAnnotation = annotation;
	}
	 ...
	 */
	private final LoadingCache<Class<?>, ImmutableList<Method>> subscriberMethodsCache = CacheBuilder.newBuilder().weakKeys().build(new CacheLoader<Class<?>, ImmutableList<Method>>() {
		@Override
